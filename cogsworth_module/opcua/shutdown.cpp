...
void RegisterSignalHandler() { SetConsoleCtrlHandler((PHANDLER_ROUTINE)CtrlHandler, TRUE); }
...
char* getAppPath() {
  ...
  strncpy_s(pszAppPath, strlen(szAppPath) + 1, szAppPath, strlen(szAppPath));
  return pszAppPath;
}
...
char* getBindingUri() {
  ...
  if (GetEnvironmentVariableA("CHAMBERLAIN_MAIN_OPCUA_BINDING_URI", buffer, buffSize)) {
    return buffer;
  } else {
...
char* getTracePath() {
  const DWORD buffSize = 65535;
  static char buffer[buffSize];
  if (GetEnvironmentVariableA("CHAMBERLAIN_COGSWORTH_LOG_FOLDER", buffer, buffSize)) {
    return buffer;
  } else {
    return getAppPath();
  }
