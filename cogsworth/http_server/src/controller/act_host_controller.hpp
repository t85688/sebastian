 ...
 */
class ActHostController : public oatpp::web::server::api::ApiController {
 private:
 ...
 public:
  typedef ActHostController __ControllerType;
 ...
 public:
   ...
   */
  ActHostController(OATPP_COMPONENT(std::shared_ptr<ObjectMapper>, objectMapper))
 ...
 public:
   ...
   */
  static std::shared_ptr<ActHostController> createShared(OATPP_COMPONENT(std::shared_ptr<ObjectMapper>, objectMapper)) {
    return std::make_shared<ActHostController>(objectMapper);
  }

 public:
  // TODO Insert Your endpoints here !!!

  ENDPOINT_INFO(GetAdapterList) {
    info->summary = "Get host adapters";
    info->addSecurityRequirement("my-realm");
    info->addTag("Host");
    info->description = "This RESTful API only permit for [Admin, Supervisor, User]";
    info->addResponse<Object<ActHostAdapterListDto>>(Status::CODE_200, "application/json");
  ...
  ENDPOINT("GET", QString("%1/adapter-list").arg(ACT_API_PATH_PREFIX).toStdString(), GetAdapterList,
    ...
    ActHostAdapterList adapter_list;

    QMutexLocker core_lock(&act::core::g_core.mutex_);

    act_status = act::core::g_core.GetHostAdapters(adapter_list);
    ...
    return createResponse(Status::CODE_200, adapter_list.ToString().toStdString());
  }
};
