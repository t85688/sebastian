...
type IServicePlatform interface {
	StartServicePlatform(connId string)
...
type ServicePlatform struct {
	...
	onAuthResponse   func(connId string, authResult domain.ServicePlatformAuthResp)
...
func (s *ServicePlatform) StartServicePlatform(connId string) {
	s.cancelTask(connId)

	ctx := s.addTask(connId)

	res := core.RequestServicePlatformDeviceCode(ctx)
	if !res.IsSuccess() {
		s.onFailedResponse(connId, res.ErrorMessage)
		return
	}

	data, ok := res.Data.(domain.ServicePlatformAuthResp)
	if !ok {
		s.onFailedResponse(connId, "Unexpected service platform auth response data")
		return
	}

	s.addDeviceCode(connId, data.DeviceCode)
	s.onAuthResponse(connId, data)

	go s.PollingServicePlatformToken(connId, ctx, data)
}
...
func (s *ServicePlatform) PollingServicePlatformToken(connId string, ctx context.Context, auth domain.ServicePlatformAuthResp) {
	...
	for {
		...
		case t := <-ticker.C:
			...
			token, exists := core.GetServicePlatformToken(auth.DeviceCode)
			if exists {
				...
				res := core.PostServicePlatformToken(ctx, auth.DeviceCode)
				if !res.IsSuccess() {
					s.onStatusResponse(connId, false)
				} else {
					ticker.Reset(1 * time.Minute)
