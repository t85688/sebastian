...
func (locator *Locator) executeDevicesLocator(orderedDeviceIdList []int64, deviceIdMap map[int64]string, duration uint32) error {
	logger.Info("Start executeDevicesLocator")
	...
	for index, id := range orderedDeviceIdList {
		if mafDeviceId, ok := deviceIdMap[id]; ok {
			...
			_, code, err := dmManager.SetLocator(mafDeviceId, cfg)
			if code >= common.MafGenericErrorBase || err != nil {
				locator.onDeviceResultProgressResponse(locator.wsConnId, locator.wsOpCode, wscommand.OperationsDeviceResult{
					...
					Status:       "Failed",
					Progress:     progress,
					ErrorMessage: "Trigger Locator failed",
				})
				errMsg := ""
				if err != nil {
					errMsg = err.Error()
				}
				logger.Info(fmt.Sprintf("Trigger %v Locator failed(code:%v, err:%v)\n", id, code, errMsg))
				...
				logger.Info(fmt.Sprintf("Trigger %v Locator success\n", id))
			}
		} else {
