...
func StartServicePlatform(ctx *wscontext.Context, data any) {
	...
	wsCmd, ok := data.(wscommand.ServicePlatformCommandSchema)
	if !ok {
		logger.Errorf("Invalid data type for StartServicePlatform: %T", data)
		return
	}
	...
	servicePlatform.StartServicePlatform(ctx.ConnId, wsCmd.DeviceCode)
}
