...
func (dispatcher *Dispatcher) ParseMessage(ctx *wscontext.Context) (any, bool, error) {
	...
	case wscommand.ActWSCommandStopFirmwareUpgrade:
		...
		commandData = baseCmd
		return commandData, true, nil
	case wscommand.ActWSCommandStartServicePlatform:
		servicePlatformCmd := wscommand.ServicePlatformCommandSchema{}
		err := json.Unmarshal(ctx.Msg, &servicePlatformCmd)
		if err != nil {
			logger.Warnf("[WS_ID: %s] Error unmarshalling ServicePlatformCommandSchema: %w", ctx.GetConnId(), err)
			return nil, false, err
		}
		commandData = servicePlatformCmd
		return commandData, true, nil
