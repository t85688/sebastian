...
func GetPowerDeviceProfiles() (*domain.PowerDeviceProfiles, statuscode.Response) {
	// 設定目標 API URL
	url := fmt.Sprintf("%s/%s/power-devices", GetCogsworthHttpEndpoint(), GetCogsworthAPIPathPrefix())
...
func GetSoftwareLicenseProfiles() (*domain.SoftwareLicenseProfiles, statuscode.Response) {
	// 設定目標 API URL
	url := fmt.Sprintf("%s/api/v2/web/cogsworth/software-license-profiles", GetCogsworthHttpEndpoint())
	...
	var profiles domain.SoftwareLicenseProfiles
	err := json.Unmarshal(data, &profiles)
	if err != nil {
		// 若解析失敗，回傳 400 BadRequest
		return nil, statuscode.StatusBadRequest(err.Error(), statuscode.MessageCode_HttpRequestSuccess)
