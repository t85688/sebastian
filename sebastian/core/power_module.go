...
func GetPowerModules() (*domain.PowerModules, statuscode.Response) {
	// 設定目標 API URL
	url := fmt.Sprintf("%s/%s/power-modules", GetCogsworthHttpEndpoint(), GetCogsworthAPIPathPrefix())
	...
	var modules domain.PowerModules
...
func GetPowerModuleNameIdMap() (map[string]int64, statuscode.Response) {
	resultMap := make(map[string]int64) // ModelName, Id

	modules, status := GetPowerModules()
	if !status.IsSuccess() {
		return resultMap, status
	}

	for _, module := range modules.PowerModuleMap {
